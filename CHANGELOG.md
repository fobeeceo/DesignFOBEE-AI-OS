...
### 책 집필 도구 — 판매용 단행본으로 확장 (2026-09-23, 대표 지시 "팔리는 책으로")
- **판매용은 자서전과 출발점이 다르다**: 자서전은 "내가 무엇을 겪었나"에서, 판매용 책은 "누가 왜 돈을 내고 사는가"에서 시작한다. 그래서 흐름을 3부로 다시 짰다 — ① 기획(시장 기획 → 아이디어 → 기획서 → 제목) ② 집필(목차 → 장 설계 → 초안 → 이어 쓰기 → 편집자 검토 → 문체 견본) ③ 출간 준비(판매 문안 → 투고 원고 Word).
- **시장 기획(신규)**: 대상 독자 · 독자의 문제 · 경쟁 도서 · 저자 정보를 작가가 적으면 포지셔닝 한 문장 · 독자 약속 3가지 · 서점 검색 키워드 10개 · 기획의 약점을 받는다. 이후 모든 단계 프롬프트에 이 정보가 따라간다.
- **제목·부제(신규)**: 약속형·호기심형·정체성형·키워드형 10개 + 추천 3개와 근거.
- **목차 = 광고**: 장 제목만 읽어도 사고 싶게, "서론·개요" 같은 제목 금지.
- **장 구성 공식**: 비소설은 첫 문단 훅 → 문제 → 핵심 개념 → 사례 → 실행 도구 → 한 줄 정리 + 다음 장 연결. 소설은 장 끝마다 질문·위기·반전. 장 설계·초안·검토가 같은 공식을 본다.
- **목표 분량 자동 계산**(§0-2 원칙 5): 목표 쪽수(기본 240쪽) × 660자 ÷ 장 수 = 장별 목표 글자 수. 한 쪽 660자는 자서전 코너와 같은 값을 쓰도록 `lib/memoir/pageSize.ts`로 분리했다(§14-A ⑥).
- **이어 쓰기(신규)**: 한 번 호출로는 한 장(1만 자 이상)을 다 쓰지 못한다. 약 5,000자씩 쓰고, 지금까지 쓴 끝부분을 넘겨 이어 쓴다. 장별 진행 막대로 목표 대비 분량을 보여준다.
- **편집자 검토(신규)**: 원고를 대신 고치지 않는다. 점수표(첫 문단 훅·독자 약속·구체성·실행 가능성·리듬·군더더기·다음 장 끌림) · 먼저 고칠 5가지(원문 인용) · 잘라낼 부분 · 사실 확인 목록.
- **판매 문안(신규)**: 뒷표지 카피 · 서점 책 소개(한 줄/200자/800자) · 저자 소개 · 출판사 투고 기획서.
- **투고 원고 Word(신규)**: 표지(제목·부제·지은이) → 차례 → 장마다 새 쪽.
- **정직 규칙**(§0-2 원칙 2·3): AI는 실제 책 제목·판매량·순위를 지어낼 수 있다. 경쟁 도서는 작가가 서점에서 직접 찾아 적은 것만 비교하고, "베스트셀러·최초·유일" 같은 확인할 수 없는 표현, 없는 추천사·이력은 쓰지 않게 했다. 저자 소개는 「저자 정보」 칸의 사실만 쓴다.
- **진행 체크리스트**: 8개 단계의 완료 여부를 저장값에서 계산해 맨 위에 보여준다.
- **예전 저장값 호환**: 기존 원고(v1)는 그대로 열린다(빠진 칸은 빈 값으로 채움, 브라우저에서 확인).
- **검증**: `npm run qa` 통과(160건, 책 집필 19건) · Chromium 390px에서 10단계 전체 흐름·체크리스트·분량 계산·Word 내려받기(표지·차례·본문·쪽 나눔 XML 확인)·가로 넘침 없음·페이지 오류 0건.
- **확인하지 못한 것**: ① 실제 Gemini 응답 품질(키 없음 — 응답을 가짜로 넣어 흐름만 확인) ② 헤드리스 크롬에서는 한글 파일명이 "download"로 저장됐다. 영문 이름은 정상이고, 자서전 Word 내보내기도 같은 방식이다. 실제 크롬·사파리에서 파일명을 확인해야 한다 ③ 아이폰 사파리 실기기.

### 책 집필 도구 `/book-writer` — 기획부터 초안까지 단계별로 쓰는 AI 도구 (2026-09-23, 대표 지시)
- **흐름**: The Nerdy Novelist(Jason Hamilton) 「How to Write a GOOD Book with AI」의 프랙탈 방식 — 아이디어 5가지 → 기획서(스토리 바이블) → 전체 목차 → 장별 장면 분해(12~15단계) → 초안 → 문체 견본(Human Touch). 목차 없이 한 장면(400~800자)씩 이어 쓰는 **즉흥 집필** 모드도 있다.
- **다음 단계는 언제나 작가가 고친 글을 기준으로 쓴다**: 각 단계 결과는 편집 가능한 칸에 들어가고, 이미 쓴 글을 AI 결과로 덮어쓰기 전에 `confirm`으로 묻는다.
- **단계마다 두 버튼**: 「AI로 만들기」(서버 Gemini, 자서전 코너와 같은 무료 횟수·내 API 키) / 「Claude용 프롬프트 복사」(Claude 프로젝트에 붙여넣기). 두 버튼은 같은 함수(`lib/bookwriter/prompts.ts`)로 프롬프트를 만든다 — 복사본과 실제 호출이 달라지지 않는다(§14-A ⑥).
- **비소설 사실 규칙**(§0-2 원칙 2): 「근거 자료」 칸에 넣은 수치·출처만 인용하고, 없으면 `[확인 필요: …]`로 비워두게 한다. 저자 경험담도 기획서에 적힌 것만 쓴다.
- **저장**: 이 브라우저에만(localStorage) — 원고 `.md` 내려받기, 백업 `.json` 내려받기/불러오기. 서버 저장은 DB 마이그레이션이라 승인 대상(§0-2 원칙 6).
- **대외 노출 보류**: 메뉴·사이트맵에 넣지 않고 `noindex`로 두었다. 공개 여부는 대표 확인 후(AI-DEV-RULES §2).
- **검증**: `npm run qa`(lint·tsc·build·test) 통과 · 테스트 11건 신규 · `next start` + Chromium(390px)으로 6단계 전체 흐름·새로고침 후 유지·클립보드 프롬프트·가로 넘침 없음·JS 끈 상태 제목 노출 확인. 키 없음 → `NO_API_KEY` 안내 실제 확인.
- **확인하지 못한 것**: 실제 Gemini 응답(이 환경에 `GEMINI_API_KEY` 없음 — 화면 흐름은 응답을 가짜로 넣어 확인), 실제 아이폰 사파리.
...
- **300쪽 계산을 화면에 공개한다**: 한 쪽 = 660자. 대표 원고 PDF 본문 49,517자 / 75쪽 ≈ 660에서 나온 값이고, 신국판 단행본 통상치(600~700자/쪽)와도 맞는다. 156문항 × 1,269자 = 197,964자 ≈ 300쪽. 화면의 모든 수치는 배열에서 계산하므로 질문을 늘리면 저절로 맞는다(§14-A ①). "실제 인쇄 쪽수는 판형·글자 크기·사진에 따라 달라집니다"를 함께 적었다(§0-2 원칙 3).
- **말로 답한다**: `VoiceInput`이 브라우저 음성 인식(Web Speech API, `ko-KR`)으로 실시간 받아쓴다. 음성을 서버로 보내지 않아 비용도 개인정보 이동도 없다. 지원하지 않는 브라우저에서는 버튼을 감추고 **키보드의 마이크 버튼**을 안내한다 — 이 경로가 실제로는 어느 기기에서나 동작한다.
- **원고는 이 기기 안에만 둔다**(`lib/memoir/storage.ts`, localStorage): 자서전에는 회복·가족·신앙까지 들어간다. 기본값을 "내 폰에만"으로 잡으면 로그인도 필요 없다 — 주 사용자가 40~80대다. 대신 브라우저 기록을 지우면 사라지므로 **내보내기 버튼을 화면 맨 위 눈에 띄는 자리**에 두고 반복해 권한다. 서버 동기화가 필요해지면 이 모듈만 어댑터로 바꾸면 된다(DB 마이그레이션은 승인 대상, §0-2 원칙 6).
- **AI는 묻기만 하고 사실을 만들지 않는다**: `/api/memoir/followup`은 답변에 실제로 나온 내용만 근거로 꼬리질문 3개를 만든다. `/api/memoir/polish`는 문장만 다듬고 "원문에 없는 사실·숫자·이름·감정을 절대 만들어 넣지 마세요"를 프롬프트 첫 줄에 둔다. 다듬은 글은 원본을 덮어쓰지 않고 나란히 보여주고 사용자가 고른다(§14-A ②⑤).
- **두 버튼을 누를 때만 답변이 서버를 거친다**는 사실을 버튼 바로 아래 명시했다.
- **기존 원고 가져오기**: 이미 써둔 글을 장을 골라 붙여넣으면 원고에 그대로 합쳐진다. 백업 JSON 복원은 덮어쓰기이므로 `confirm`으로 한 번 받는다.
- **JS가 실패해도 질문이 보인다**(§0-6): `opacity: 0` 기본값을 쓰지 않았다. 배포본 HTML을 직접 확인해 랜딩 본문과 쓰기 화면의 첫 질문이 서버 렌더 결과에 들어 있는 것을 확인했다. 본문 15px 이상, 버튼 높이 48~56px.
- **검증**: `npm run lint` exit 0 · `npx tsc --noEmit` exit 0 · `npm run build` exit 0(`/memoir`·`/memoir/write`·`/memoir/book` 3개 라우트 + API 2개 생성) · `npm run test` **115건 PASS**(자서전 18건 신규) · `next start`로 띄워 세 페이지 200 응답과 SSR 본문·계산 수치(156/13/5/1,269/197,964/300/660) 실제 확인.
- **확인하지 못한 것(정직 기록, §0-2 원칙 7)**: ① 실제 아이폰 사파리에서의 음성 인식 동작 — 이 컨테이너에는 마이크도 iOS 기기도 없다. 배포 후 실기기 확인이 필요하다. ② 두 AI API의 실제 응답 — `GEMINI_API_KEY`가 이 환경에 없어 호출하지 못했다. 라우트의 입력 검증·오류 처리 경로만 코드로 확인했다. ③ 실제 인쇄 쪽수 — 판형이 정해지지 않아 환산값이다.
...
- Notion "AI 제안함" 스키마를 직접 조회해 `적용범위`(select: 홈페이지 UI/콘텐츠/기타)·`제안자`(select: AI 웹디자인전략가 기존 존재)·`처리채널`(select: Notion 직접 기존 존재) 옵션이 이미 갖춰져 있음을 확인 후 신규 옵션 추가 없이 그대로 사용.
- **검증**: `POST /api/hq/design-trends`를 실제 서비스 토큰으로 호출(오늘의집 실fetch 성공, 스타벅스코리아는 이번엔 본문이 너무 짧아 접근실패로 정직 반환 — 코드가 이 부분 그레이스풀 처리함을 실행으로 확인) → 응답 스키마(`트렌드종합`·`우리대비제안`·`접근실패`)가 Code 노드 파싱 로직과 일치함을 직접 확인. n8n 워크플로 자체는 생성만 하고 실행은 하지 않음(Notion credential 미연결 상태라 마지막 노드에서 실패할 것이 확실하므로 불필요한 반복 실행 방지).
- **정직한 범위 고지**: 워크플로는 Notion Internal Integration credential(CEO 본인 계정 인증 필요, [INSTALL.md](INSTALL.md) §11)이 연결되기 전까지 inactive — 메뉴전략 워크플로와 동일한 기존 블로커(TODO.md 기록). 이번 사이클에 새 credential 요구는 추가하지 않았음(기존 §11 절차 그대로 재사용).
- 코드 변경 없음(n8n 워크플로 정의만 추가돼 저장소 밖 n8n 자체 DB에 저장됨) — 참고용으로 `npm run build`/`lint` 재확인.
