...
- **P2 SUPPLIER_MASTER 고도화**: Notion에 연결된 Google Doc(v1.2)을 직접 읽어보니 실제 확정 거래처 데이터(뉴디스코리아·서울우유·고흥식품·정선식품·제원인터내셔날·본사제작)가 있었음 — 이걸 근거로 거래처/단위/공급단가/최소주문수량/LeadTime/발주요일/최근단가변경일/비고 9컬럼 구조로 확장한 v1.3을 신설(Drive 새 파일 생성, v1.0~1.2는 그대로 유지 — 이 Drive 커넥터에 파일 수정 도구가 없어 기존 파일을 고치는 대신 회사의 기존 버저닝 관례를 따라 새 버전 파일 생성). Notion 페이지·`erp_engine.py SUPPLIER_MAP`도 동일 구조로 동기화. 단가·MOQ 등 원본에 없는 값은 전부 "확인대기"로 남김(추측 금지).
- **P3 자동 발주 고도화**: `purchase_orders()`에 부족률(%)·예상금액(공급단가 확인시만 계산) 추가. `supplier_order_totals()` 신설 — 거래처별 합계, 금액 미확정시 "확인불가(단가 미확정)" 정직 표기. 최종 구조(재고→안전재고→AI추천→Notion승인→대표승인→발주서생성→이메일초안생성→최종발송)의 마지막 두 단계는 `auto-order-recommender` SKILL.md에 절차로 추가(승인된 건만 대상, Gmail `create_draft`만 사용, 발송 도구 호출 금지). 현재 승인된 건이 없어 이메일 초안 경로는 코드/절차만 준비되고 실행 미검증 상태(정직 기록) — 공급처 이메일 주소 자체도 SUPPLIER_MASTER에 없어 그 경우 수신처를 비우고 "확인 필요"를 본문에 명시하도록 함.
- **P4 Dashboard KPI**: `/hq` 첫 화면을 지시된 7개 KPI로 재구성. "발주 승인 대기"는 Notion의 실제 승인 상태가 아니라 ERP 추천 건수를 쓰는 것이라는 점을 sub 텍스트에 명시(라이브 Notion 연동이 이 Next.js 앱에 없어 정확히 같은 값이 아닐 수 있음 — 과장 없이 그대로 노출). AI실패건수·오늘일정·AI작업완료율·전국매장현황 4개는 아직 실데이터 연결이 없어 점선 테두리 카드로 "연결 필요"만 표시(가짜 수치 없음).
- **P5 OAuth 점검(실측)**: Gmail(`list_drafts`)·Google Calendar(`list_calendars`, `list_events`)를 이 세션에서 직접 호출해 연결 확인 — 다만 이건 n8n이나 Next.js 앱 자체의 자격증명이 아니라 **이 Claude Code 세션 자체의 커넥터**라 완전히 같은 것은 아님(n8n 워크플로 활성화에는 별도로 CEO 본인 계정 credential이 여전히 필요, 기존 TODO 그대로 유지). Telegram은 Bot 자체가 없어 미연결. Notion·GitHub·Vercel은 기존과 같이 정상.
- **P6 GitHub 정책**: Private 전환은 2026-07-28에 이미 CEO 승인을 받았으나 이 세션에 GitHub CLI·API 토큰이 없어 실행 수단이 없음 — Owner Action Required로 TODO에 유지.
- **P7 AI 직원 점검**: AI QA(`npm run qa`)·AI Audit(`npm run audit`)·AI SEO Manager 등(`npm run qa:extended`)·AI Documentation(`check-docs-sync.js`) 4건을 오늘 실제로 재실행해 정상 확인(신규 이슈 없음, 기존에 알려진 label.tsx 오탐 1건만 유지). 코드 실행에 실제 비용(Gemini API 등)이 드는 나머지 9개 정규직 역할(Blog Writer·Trend Researcher 등)은 불필요한 과금을 피하기 위해 오늘 재실행하지 않음 — [AI-STAFF-POLICY.md](AI-STAFF-POLICY.md) §7(2026-07-23 마지막 실측)을 그대로 최신으로 인정.
- **P8 Supabase 점검(실측, 대시보드 없이 확인)**: `npx prisma db pull`로 로컬 `DATABASE_URL`을 이용해 실제 연결을 시도 → `FATAL: tenant/user postgres.edwnvwawckfdarsxobah not found`. 이는 프로젝트가 여전히 일시정지 상태거나 접근 불가함을 나타내는 Supabase 커넥션 풀러의 실제 오류(추측이 아니라 직접 실행한 결과). 프로덕션 `/login`의 브라우저 콘솔 에러(Supabase URL/key 없음)와 함께 종합하면, 이 프로젝트는 어제(2026-07-28) CEO에게 확인을 요청한 이후에도 여전히 미해결 상태로 판단됨.
- **P9 QA/Audit/Build**: 이번 사이클의 모든 코드 변경마다 `npm run qa`·`npm run audit` 실행 후 PASS 확인하고서만 커밋(중간에 실패한 사례 없음, 재작업 불요). 최종 1회 더 재확인.
- **P10 문서 동기화**: `check-docs-sync.js` 0건. CHANGELOG·DECISION-LOG·TODO 갱신.
- **의도적으로 하지 않은 것(정직 기록)**: ①Supabase 프로젝트 재개는 대시보드 접근 권한이 없어 대신 할 수 없음(CEO 액션) ②GitHub Private 전환은 도구가 없어 실행 못함(CEO 액션) ③9개 정규직 AI 역할 재실행은 비용 발생 우려로 보류 ④이메일 초안 경로는 승인된 발주 건이 없어 종단 검증 못함.
...
- **미결 항목**: 홈에서 제거된 `components/home/MenuSection.tsx`는 아직 파일로 남아 audit에서 "미사용 코드"로 잡힘 — 삭제는 별도 승인 필요. Vercel 배포는 대표님이 직접 최종 확인 후 진행.

## 2026-07-27 — 신규 파이프라인 첫 실전 사례: 포트폴리오 사진 교체+갤러리+회사소개 페이지
- **배경**: 위 §"실행 권한 원칙 부분 개정" 직후 CEO가 채팅에서 직접 홈페이지 변경을 요청(포트폴리오 사진 스크린샷 첨부 + Drive 실사진 폴더 + 참고 사이트 2곳). "지금 바로 채팅으로 요청하면 그 자리에서 Notion 승인 등록→구현까지" 경로로 실제 진행.
- **요청 내용**: ①"은평본점" 라벨 사진이 실제로는 "단대점" 사진이라는 CEO 지적 → 정확한 매핑을 CEO에게 재확인 후 두 파일 내용 교체(추측 금지 원칙 적용, 사진만 보고 스스로 판단하지 않음). ②Google Drive `GBRICK FRANCHISE SYSTEM/디자인포비작업사진`(83장, 파일명에 프로젝트 메타데이터 없음) 중 9장을 실제로 열람해 선별, 기존 `sync-images.js` 파이프라인(WebP 변환+최적화+썸네일+Gemini ALT) 그대로 재사용해 처리. ③inmidas.co.kr(회사소개 참고)·rehouse.ae(호버 인터랙션 참고) 요청 — 둘 다 접근 실패(Chrome 확장 미연결, SSL 인증서 만료)를 CEO에게 솔직히 고지하고, 일반적인 검증된 호버 패턴(스케일업+오버레이+그림자 리프트)으로 대체 구현.
- **정직한 기록(추측 금지)**: 83장 사진에 매장명·연도·프로젝트명 메타데이터가 전혀 없어, 신규 갤러리 사진에는 "은평본점" 같은 특정 매장명을 절대 부여하지 않고 사진에서 실제로 확인되는 공간 용도(리테일 VMD·카페·오피스 등)만 카테고리로 기록(`lib/portfolio/workGallery.ts` 주석에 명시).
- **회사소개(/about) 페이지**: 새로운 사실을 만들지 않고 기존 코드베이스에 이미 검증된 SSOT(`AboutSection`의 연혁, `ServicesSection`의 사업영역, `Footer`의 대표자명·설립일·연락처)만 `lib/company/profile.ts`로 통합해 재사용 — 3개 컴포넌트 간 데이터 중복도 함께 제거.
- **파이프라인 절차 준수**: Notion "AI 제안함"에 항목 생성(id `3aa3abcb-b871-81d1-99ab-cf02fa0f7f21`, 적용범위=홈페이지 UI/콘텐츠, 상태=승인) → 구현 → `npm run qa`·`npm run audit` 통과 → `ai-proposal/3aa3abcb` 브랜치에 커밋(main 미접촉, push 안 함) → Notion 구현상태를 "구현완료(리뷰대기)"로 갱신 → main으로 복귀.
- **검증**: preview 서버로 홈(포트폴리오 스왑 확인·신규 갤러리 렌더 확인)·`/about`(연혁·통계·사업영역·연락처 텍스트 a11y 스냅샷으로 실제 값 확인)·모바일 375px 뷰포트까지 직접 확인. 콘솔 에러 0건.

## 2026-07-27 — 실행 권한 원칙 부분 개정: 홈페이지 UI/콘텐츠는 "코드+QA/Audit까지 AI, push는 사람"으로 확대
- **배경**: CEO 질문 "사이트 만들던거 이제 ai직원들이 만들수 있는거야" → 기존 원칙(2026-07-23, AI-STAFF-POLICY.md §4 "AI는 제안까지만")으로는 아니라고 답변 → CEO가 "바꾸는 것까지 결정"이라고 명시적으로 원칙 변경을 지시.
- **검토한 대안(자동화 범위)**: ①제안 승인까지만 자동, 구현은 여전히 사람/Claude Code 세션이 수동으로 ②코드 작성+QA/Audit까지 자동, 커밋은 로컬/브랜치만, push·배포는 사람 ③커밋·push까지 자동(Vercel 자동배포로 실사이트 즉시 반영). CEO가 AskUserQuestion으로 **②**를 선택(Recommended 옵션).
- **검토한 대안(적용 범위)**: ①홈페이지 UI/콘텐츠만 ②전체 코드베이스(ERP·인증·API 포함). CEO가 **①**을 선택 — 핵심 로직(결제 없음이지만 인증·데이터 스키마 등)은 여전히 사람 개입 필수로 유지, blast radius를 UI/콘텐츠로 한정.
- **결정**: AI-STAFF-POLICY.md §4에 "2026-07-27 개정" 조항 추가. 게이트는 Notion "AI 제안함" DB의 `상태=승인` AND `적용범위=홈페이지 UI/콘텐츠`. 실행 안전장치는 3중: (a) 파일 범위 제한(`app/page.tsx`·`components/home/**`만) (b) QA/Audit 실패 시 변경 되돌리고 중단 (c) `git push` 절대 금지, 로컬 브랜치(`ai-proposal/<id>`)에만 커밋 — 대표님이 리뷰 후 직접 push.
- **구현**: Notion "AI 제안함" DB에 `적용범위`·`구현상태`(대기/구현중/구현완료(리뷰대기)/실패)·`브랜치`·`구현일시` 4개 속성 추가. 매일 10:00(Asia/Seoul) 실행되는 예약 작업 `ai-proposal-implementer`(`C:\Users\user\.claude\scheduled-tasks\ai-proposal-implementer\SKILL.md`) 신설 — 대상 없으면 조용히 종료, 있으면 위 절차대로 구현 후 Notion 갱신.
- **정직한 기록**: 이 예약 작업은 로컬 Claude Code 앱이 켜져 있어야 실행된다(클라우드 상시 실행 아님) — 앱이 꺼져 있으면 다음 실행 시 대신 돈다. 아직 실제 승인된 "홈페이지 UI/콘텐츠" 제안이 없어 이 파이프라인은 아직 실전 실행 이력이 없다(다음 AI 웹디자인전략가 제안이 승인되면 최초 검증 필요).
