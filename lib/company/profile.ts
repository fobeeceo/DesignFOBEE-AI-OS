 ...
 *
 * ⚠️ 이 값만 유일하게 자동 계산이 불가능하다. 홈페이지에 공개한 사진(WORK_GALLERY)은
 *    전체 실적의 일부일 뿐이라 사진 개수로는 실제 실적을 셀 수 없기 때문이다.
 *    출처가 업무지명원이므로, 지명원이 갱신되면 이 값도 함께 수정한다.
 *    추정치이므로 화면에는 "약"을 붙여 단정하지 않는다.
 */
export const TOTAL_PROJECTS = 250;
