/**
 * 헤르메스(Hermes) — AI HQ 전달 계층(Delivery Layer) 공통 타입.
 *
 * 헤르메스는 전령이다. 내용을 새로 만드는 역할이 아니라, 이미 사실인 것을 *올바른 상대에게*
 * *올바른 경로로* 옮기는 역할이다. 그래서 이 계층의 중심 개념은 "봉투(Envelope)" 하나다.
 * 내부 신호든 대외 문의든 부서 요청이든 외부 소식이든, 전부 봉투로 정규화한 다음 같은
 * 라우팅 규칙을 통과한다.
 *
 * 절대 원칙 — 헤르메스는 스스로 보내지 않는다.
...
/** 봉투가 어디서 왔는가 — 헤르메스의 4가지 업무에 각각 대응한다. */
export type Origin =
