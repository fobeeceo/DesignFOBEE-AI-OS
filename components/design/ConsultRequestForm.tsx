...
/**
 * STEP 8: AI 디자인 결과(이미지+설명+견적)를 첨부한 상담 신청 폼.
 * 홈페이지 ContactForm과 동일한 leadSchema/POST /api/leads를 재사용하되,
 * designImageId와 source="design_studio"를 함께 전송한다.
 */
export function ConsultRequestForm({ designImageId }: ConsultRequestFormProps) {
  ...
  const {
    ...
    defaultValues: { source: "design_studio", designImageId },
  });
  ...
  async function onSubmit(values: LeadFormValues) {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ ...values, source: "design_studio", designImageId }),
      });
    ...
    return (
      <div className="rounded-2xl border border-border bg-muted/40 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          AI 디자인 결과가 함께 전달되었습니다. 빠른 시일 내에 담당자가 연락드리겠습니다.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Textarea
          id="message"
          placeholder="희망 일정, 예산, 궁금한 점 등을 자유롭게 남겨주세요."
          {...register("message")}
        />
      ...
      <Button type="submit" size="lg" disabled={state === "submitting"}>
        {state === "submitting" ? "전송 중..." : "이 디자인으로 상담 신청하기"}
      </Button>
    </form>
  );
}
